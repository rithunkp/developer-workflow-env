...
def task_schema(task: str, observation: dict) -> dict:
    ...
    return {
        ...
        "rules": [
            ...
            "CRITICAL: When issuing a 'fix', your 'original' field MUST perfectly match the exact text from the observation's code diff (including any randomized variable names).",
    ...
    for issue in KNOWN_CODE_ISSUES:
        if issue["line"] not in state.fixed_lines:
            ...
            return {
                "phase": "fix",
                "line": issue["line"],
                "original": get_exact(issue["original_contains"]),
                "fixed": issue["fixed"],
...
def choose_action(task: str, observation: dict, last_error: str | None, last_action: str | None, state: HeuristicState) -> tuple[dict, str]:
    ...
    # Fallback to heuristic ONLY if the LLM completely fails or produces invalid JSON
